package com.ss.sprintflink.util;
...
//    }
     ...
     * @param clazz     数据实体类
     * @param collName  集合名称
     * @param map       Map<"查询条件key"，Map<大于/等于/小于,值></>> map 若 keys/values 为null,则查询集合中所有数据
     * @param pageNo    当前页
     ...
     * @return
     */
    public static PageModel findSortPageCondition(Class<?> clazz, String collName, Map<String, Map<String, Object>> map,
                                                  int pageNo, int pageSize, Sort.Direction direction, String sortField) {

        Criteria criteria = getCriteriaNew(new Criteria(), map);
     ...
     * @return
     */
    private static Criteria getCriteriaNew(Criteria criteria, Map<String, Map<String, Object>> map) {
        if (map == null) {
            return null;
        }
        int i = 0;
        for (String key : map.keySet()) {
            ...
            for (String valKey : val.keySet()) {
                //is 等于
                if (valKey == "is") {
                    criteria = criteria.is(val.get(valKey));
                }
                //lt <
                if (valKey == "lt") {
                    criteria = criteria.lt(val.get(valKey));
                }
                //lte <=
                if (valKey == "lte") {
                    criteria = criteria.lte(val.get(valKey));
                }
                //gt >
                if (valKey == "gt") {
                    criteria = criteria.gt(val.get(valKey));
                }
                //gte >=
                if (valKey == "gte") {
                    criteria = criteria.gte(val.get(valKey));
                }
            }
        }
        return criteria;

...
public class HBaseUtils {
     ...
     * @param pairList         列标识和值的集合
     */
    public static boolean putRow2(String tableName, String rowKey, String columnFamilyName, List<Pair<byte[], byte[]>> pairList) {
        try {
            Table table = connection.getTable(TableName.valueOf(tableName));
            Put put = new Put(Bytes.toBytes(rowKey));
            pairList.forEach(pair -> put.addColumn(Bytes.toBytes(columnFamilyName), pair.getKey(), pair.getValue()));
            table.put(put);
            table.close();
        } catch (IOException e) {
            e.printStackTrace();
